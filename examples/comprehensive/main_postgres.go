...
type EmailTaskPG struct {
	To      string `json:"to"`
	Subject string `json:"subject"`
	Body    string `json:"body"`
}

func (EmailTaskPG) Kind() string { return "email_pg" }

// ReportTask - Scheduled task for generating reports
type ReportTaskPG struct {
	ReportType string    `json:"report_type"`
	StartDate  time.Time `json:"start_date"`
	EndDate    time.Time `json:"end_date"`
}

func (ReportTaskPG) Kind() string { return "report_pg" }

// NotificationTask - Delayed task for sending notifications
type NotificationTaskPG struct {
	UserID  string `json:"user_id"`
	Message string `json:"message"`
	Type    string `json:"type"`
}

func (NotificationTaskPG) Kind() string { return "notification_pg" }

// HealthCheckTask - Recurring task (every hour)
type HealthCheckTaskPG struct {
	ServiceName string `json:"service_name"`
	Endpoint    string `json:"endpoint"`
}

func (HealthCheckTaskPG) Kind() string { return "health_check_pg" }

// DataSyncTask - Task with retry logic
type DataSyncTaskPG struct {
	SourceID      string `json:"source_id"`
	DestinationID string `json:"destination_id"`
}

func (DataSyncTaskPG) Kind() string { return "data_sync_pg" }

// BatchProcessTask - Batch processing task
type BatchProcessTaskPG struct {
	BatchID  string   `json:"batch_id"`
	ItemIDs  []string `json:"item_ids"`
	Priority int      `json:"priority"`
}

func (BatchProcessTaskPG) Kind() string { return "batch_process_pg" }

// AlwaysFailTask - Task that always fails (for testing retry exhaustion)
type AlwaysFailTaskPG struct {
	Reason string `json:"reason"`
}

func (AlwaysFailTaskPG) Kind() string { return "always_fail_pg" }
...
type EmailWorkerPG struct {
	ergon.WorkerDefaults[EmailTaskPG]
}

func (w *EmailWorkerPG) Work(ctx context.Context, task *ergon.Task[EmailTaskPG]) error {
...
func (w *EmailWorkerPG) Timeout(task *ergon.Task[EmailTaskPG]) time.Duration {
	return 30 * time.Second
}

// ReportWorker generates reports
type ReportWorkerPG struct {
	ergon.WorkerDefaults[ReportTaskPG]
}

func (w *ReportWorkerPG) Work(ctx context.Context, task *ergon.Task[ReportTaskPG]) error {
...
type NotificationWorkerPG struct {
	ergon.WorkerDefaults[NotificationTaskPG]
}

func (w *NotificationWorkerPG) Work(ctx context.Context, task *ergon.Task[NotificationTaskPG]) error {
...
type HealthCheckWorkerPG struct {
	ergon.WorkerDefaults[HealthCheckTaskPG]
}

func (w *HealthCheckWorkerPG) Work(ctx context.Context, task *ergon.Task[HealthCheckTaskPG]) error {
...
type DataSyncWorkerPG struct {
	ergon.WorkerDefaults[DataSyncTaskPG]
	attemptCount int
}

func (w *DataSyncWorkerPG) Work(ctx context.Context, task *ergon.Task[DataSyncTaskPG]) error {
	w.attemptCount++
...
func (w *DataSyncWorkerPG) MaxRetries(task *ergon.Task[DataSyncTaskPG]) int {
	return 5
}

func (w *DataSyncWorkerPG) RetryDelay(task *ergon.Task[DataSyncTaskPG], attempt int, err error) time.Duration {
	// Exponential backoff: 2s, 4s, 8s, 16s, 32s
	return time.Duration(1<<uint(attempt)) * time.Second
}

// BatchProcessWorker processes batches
type BatchProcessWorkerPG struct {
	ergon.WorkerDefaults[BatchProcessTaskPG]
}

func (w *BatchProcessWorkerPG) Work(ctx context.Context, task *ergon.Task[BatchProcessTaskPG]) error {
...
func main() {
	...
	log.Println("🚀 Ergon Comprehensive Example - PostgreSQL Version - Starting...")
	...
	if connStr == "" {
		...
		log.Printf("⚠️  Using default DATABASE_URL: %s", connStr)
	}
	...
	log.Println("✅ PostgreSQL store ready")
	...
	ergon.AddWorker(workers, &EmailWorkerPG{})
	ergon.AddWorker(workers, &ReportWorkerPG{})
	ergon.AddWorker(workers, &NotificationWorkerPG{})
	ergon.AddWorker(workers, &HealthCheckWorkerPG{})
	ergon.AddWorker(workers, &DataSyncWorkerPG{})
	ergon.AddWorker(workers, &BatchProcessWorkerPG{})
	...
	log.Println("\n" + strings.Repeat("=", 70))
	log.Println("🎯 DEMONSTRATING ALL ERGON FEATURES (PostgreSQL Backend)")
	log.Println(strings.Repeat("=", 70))

	// FEATURE 1: Standard Task
	log.Println("\n1️⃣  Standard Task - Immediate execution")
	task1, _ := ergon.Enqueue(client, ctx, EmailTaskPG{
		To:      "user@example.com",
	...
	task2, _ := ergon.Enqueue(client, ctx, NotificationTaskPG{
		UserID:  "user-123",
	...
	task3, _ := ergon.Enqueue(client, ctx, ReportTaskPG{
	...
	task4, _ := ergon.Enqueue(client, ctx, HealthCheckTaskPG{
	...
	task5, _ := ergon.Enqueue(client, ctx, DataSyncTaskPG{
		SourceID:      "db-primary",
	...
	task6a, _ := ergon.Enqueue(client, ctx, BatchProcessTaskPG{
		BatchID:  "batch-low",
		ItemIDs:  []string{"item1", "item2"},
		Priority: 1,
	}, ergon.WithQueue("low_priority"), ergon.WithPriority(1))

	task6b, _ := ergon.Enqueue(client, ctx, BatchProcessTaskPG{
		BatchID:  "batch-high",
	...
	task8, _ := ergon.Enqueue(client, ctx, DataSyncTaskPG{
	...
	_, err = ergon.Enqueue(client, ctx, DataSyncTaskPG{
		SourceID:      "unique-source",
		DestinationID: "unique-dest",
	}, ergon.AtMostOncePerHour())
	if err != nil {
	...
	task9, _ := ergon.Enqueue(client, ctx, EmailTaskPG{
	...
	pendingCancelTask, _ := ergon.Enqueue(client, ctx, EmailTaskPG{
	...
	ergon.AddWorkerFunc(workers, func(ctx context.Context, task *ergon.Task[AlwaysFailTaskPG]) error {
	...
	failedTask, _ := ergon.Enqueue(client, ctx, AlwaysFailTaskPG{
	...
	deleteTask, _ := ergon.Enqueue(client, ctx, EmailTaskPG{
	...
	log.Println("💾 All data persisted in PostgreSQL database")
}
