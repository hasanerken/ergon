...
func main() {
	...
	if connStr == "" {
		// Default to user-provided test DSN if DATABASE_URL is not set
		connStr = "postgres://postgres:ergon123@localhost:5432/ergon?sslmode=disable"
		fmt.Printf("⚠️  Using default DATABASE_URL: %s\n", connStr)
		fmt.Println("   Set DATABASE_URL environment variable to override this for your environment")
	}
	...
	fmt.Println("\n🧪 ERGON SCHEDULING TEST - PostgreSQL - NEXT FEW MINUTES")
	fmt.Println(strings.Repeat("=", 70))
